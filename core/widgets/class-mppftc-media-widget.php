<?php
/**
 * Widget class to list featured media
 *
...
/**
 * Class MPPFTC_Media_Widget
 */
class MPPFTC_Media_Widget extends WP_Widget {
	...
	public function __construct( $name = '', $widget_options = array() ) {
		...
		if ( empty( $name ) ) {
			$name = __( '( MediaPress ) Featured Media', 'mpp-featured-content' );
		}
	...
	public function widget( $args, $instance ) {
		...
		require mpp_locate_template( array( 'loop-featured-media.php' ), false, mppftc_featured_content()->get_path() . 'templates/widgets' );
	 ...
	 *
	 * @param array $new_instance New settings value.
	 * @param array $old_instance Old settings value.
	 *
	 * @return array
	 */
	...
	public function form( $instance ) {
		...
		$defaults = array(
			'title'       => __( 'Featured Media items', 'mpp-featured-content' ),
			'component'   => 'members',
			'status'      => 'public',
			'type'        => 'photo',
			'max_to_list' => 5,
...
function mppftc_register_media_widget() {
	register_widget( 'MPPFTC_Media_Widget' );
}
add_action( 'mpp_widgets_init', 'mppftc_register_media_widget' );
